...
function parseRequirementStatus(item: string): {
	label: string;
	completed: boolean;
} {
	return {
...
function buildFallbackSummary(
	auditData: ReturnType<typeof readAuditData>,
	...
	const summary: ParsedRequirements = {
		degree: [
			...
			...parserPayload.classes_taken.map(
				(course) => `${formatCourseId(course)} — Completed`,
			),
		],
		commonCore: [],
		pluralism: [],
...
export function ViewRequirements() {
	...
	const summary =
		auditData?.requirementsSummary ?? buildFallbackSummary(auditData);
	...
	const completedCount = summary
		? REQUIREMENT_GROUPS.reduce(
				(count, { key }) =>
					count +
					(summary[key] ?? []).filter((item) => item.endsWith("— Completed"))
						.length,
				0,
			)
		: 0;
			...
			<main className="max-w-screen-2xl mx-auto px-4 lg:px-6 py-6">
				...
				<div className="mb-6">
					...
					<p className="text-sm text-gray-600">
						Completed and still-needed requirements from your confirmed
						DegreeWorks audit
					</p>
				</div>
					...
					<Card className="shadow-lg border-0 bg-white/90 backdrop-blur-sm">
						<CardContent className="flex flex-col items-center justify-center py-16 gap-4">
							...
							<p className="text-sm text-gray-700 font-medium text-center max-w-md">
								No saved audit data was found. Upload and confirm your audit to
								view your requirements here.
							</p>
							<Button
								size="sm"
					...
					<div className="grid lg:grid-cols-[360px_1fr] gap-5 items-start">
							...
							<Card className="shadow-lg border-0 bg-white/90 backdrop-blur-sm">
								<CardHeader className="pb-3">
									...
									<CardDescription className="text-sm mt-1">
										Saved from your confirmed DegreeWorks audit
									</CardDescription>
								</CardHeader>
									...
									<div>
										...
										{stillNeeded.length > 0 ? (
											...
											<p className="text-sm text-gray-500">
												No still-needed requirements were saved.
											</p>
										)}
									</div>
						...
						<Card className="shadow-lg border-0 bg-white/90 backdrop-blur-sm">
							<CardHeader className="pb-3">
								...
								<CardDescription className="text-sm mt-1">
									The same completed and still-needed items from your audit
									review
								</CardDescription>
							</CardHeader>
								...
								{REQUIREMENT_GROUPS.map((group) => ({
									...
									.map(({ label, items, color, bg, border, summaryOnly }) => (
										<section key={label}>
											...
											>
												{items.map((item, idx) => {
													const { label: itemLabel, completed } =
														parseRequirementStatus(item);
													if (summaryOnly) {
														return (
															<li
																key={`${label}-${idx}`}
													...
													return (
														...
														>
															...
															<span
																className={`text-sm ${completed ? "text-gray-800" : "text-gray-500"}`}
															>
																{itemLabel}
															</span>
														</li>
