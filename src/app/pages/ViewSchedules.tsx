...
const SCHEDULER_ERROR_MESSAGES: Record<string, string> = {
	NO_CANDIDATE_SECTIONS:
		"No matching classes were found for your current requirements and term.",
	ALL_CANDIDATES_FAIL_PREREQS:
		"We found classes, but prerequisites are not satisfied for all of them.",
	ALL_CANDIDATES_IN_BLOCKED_TIME:
		"All matching classes conflict with your unavailable time settings.",
	NO_ELIGIBLE_CANDIDATES:
		"No classes remain after applying your eligibility and time filters.",
	UNSAT_HARD_CONSTRAINTS:
		"No valid schedule satisfies all required constraints. Try relaxing filters.",
	NO_CANDIDATES_FOUND:
		"No candidate sections were found for the requested schedule.",
	UNSATISFIABLE_CONSTRAINTS:
		"The requested schedule constraints cannot be satisfied.",
	DATABASE_CONNECTION_FAILED:
		"The scheduler could not connect to its database. Please try again shortly.",
	INVALID_PAYLOAD:
		"The schedule request was malformed. Please re-upload your audit and try again.",
	SOLVER_FAILED:
		"The scheduler encountered an internal error. Please try again.",
	SOLVER_TIMEOUT:
		"Schedule generation took too long. Try relaxing your constraints or removing a pinned course.",
	SPECIFIC_COURSE_NO_ELIGIBLE_SECTIONS:
		"One or more pinned courses has no eligible sections for this term.",
};
...
const OPTIMIZATION_LABELS: Record<string, string> = {
	...
	OPT_MULTI_REQUIREMENT_TAG_PRIORITY:
		"Multi-requirement course priority active",
...
function formatMeetingDays(section: ScheduleSection): string {
	const days = [
		...new Set(section.meetings.map((m) => DAY_LABEL[m.day] ?? m.day)),
	];
	return days.join(", ");
}
...
export function ViewSchedules() {
	...
	useEffect(() => {
		...
		if (!auditData?.parserPayload) {
			setError(
				"No audit data found. Please upload your DegreeWorks audit first.",
			);
		...
		fetch(`${SCHEDULER_BASE}/api/schedule/generate`, {
			...
			.then(async (res) => {
				...
				try {
					...
					throw new Error(
						"Failed to read response body — the scheduler may have closed the connection.",
					);
				}
				clearTimeout(timeoutId);
				let data: unknown;
				try {
					...
					throw new Error(
						"Schedule response could not be parsed. The scheduler may have returned an unexpected format.",
					);
				}
				...
				if (!res.ok) {
					...
					throw new Error(
						(code && SCHEDULER_ERROR_MESSAGES[code]) ??
							msg ??
			...
			.then((data) => {
				...
				if (data.sections.length === 0 && data.error_code) {
					setError(
						SCHEDULER_ERROR_MESSAGES[data.error_code] ??
							data.error_message ??
			...
			.catch((err: Error) => {
				...
				if (err.name === "AbortError") {
					setError(
						"Schedule generation timed out. The scheduler may be busy — please try again.",
					);
				} else {
					...
					<div className="space-y-4">
								...
								<Tabs defaultValue="list" className="w-full">
													...
													<div className="flex items-start justify-between mb-3">
														<div>
															<h4 className="font-bold text-base">
																{section.course.subject_area}{" "}
																{section.course.catalog_number} —{" "}
																{section.course.course_title}
															</h4>
															<p className="text-sm text-gray-600 mt-0.5">
																Section {section.section_code} · Class #
																{section.class_num}
																{section.major_elective && (
														...
														<div className="flex gap-2 items-start">
															<Badge
																className={`text-xs px-2 py-0.5 ${SECTION_COLORS[idx % SECTION_COLORS.length].split(" ")[0].replace("bg-", "bg-")} border`}
															>
													...
													<div className="grid md:grid-cols-2 gap-3 text-sm">
														...
														<div className="flex items-center gap-2 text-gray-700">
															...
															<span>
																...
																{section.class_capacity} enrolled
															</span>
														</div>
													</div>
									...
									<TabsContent value="calendar" className="mt-4">
										<div className="text-xs text-gray-500 mb-2 flex items-center gap-1.5">
											...
											Classes span their actual meeting times · hover for
											details
										</div>
										...
										<div className="rounded-xl border border-gray-200 shadow-sm bg-white">
											...
											<div className="grid grid-cols-7 border-b border-gray-200 rounded-t-xl overflow-hidden">
												...
												{GRID_DAYS.map((day) => (
													<div
														key={day}
														className="p-3 bg-gray-50 border-r border-gray-200 last:border-r-0 text-center font-semibold text-sm"
													>
														{DAY_LABEL[day]}
													</div>
												))}
											...
											<div
												...
												style={{ height: `${TIME_ROWS.length * ROW_HEIGHT}px` }}
											>
												{/* Time label column */}
												<div className="bg-gray-50 border-r border-gray-200 relative">
													{TIME_ROWS.map((minutes) => (
														<div
															...
															style={{
																top: ((minutes - GRID_START) / 60) * ROW_HEIGHT,
																height: ROW_HEIGHT,
															}}
														>
													...
													>
														...
														{TIME_ROWS.map((minutes) => (
															<div
																...
																style={{
																	top:
																		((minutes - GRID_START) / 60) * ROW_HEIGHT,
																}}
															/>
														))}
														{/* Section blocks */}
														{schedule.sections.flatMap((section) =>
															section.meetings
																...
																.map((meeting, mi) => {
																	const top =
																		((meeting.start_time - GRID_START) / 60) *
																		ROW_HEIGHT;
						...
						<Card className="shadow-lg border-0 bg-white/90 backdrop-blur-sm">
							<CardHeader className="pb-3">
								...
								<CardDescription className="text-sm mt-1">
									Ready to register for these courses?
								</CardDescription>
							</CardHeader>
